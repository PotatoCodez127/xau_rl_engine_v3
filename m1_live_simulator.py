            ...
            if latest_15m_features is not None:
                ...
                if len(self.feature_buffer) == 30:
                    ...
                    if direction != 0:
                        ...
                        # Extract the 2 actions generated by the ONNX SAC model
                        sl_val, tp_val = action[0], action[1]
                        
                        # SL Multiplier: 0.5x to 2.0x ATR (Matches env logic)
                        sl_mult = 0.5 + ((sl_val + 1.0) * (2.0 - 0.5)) / 2.0
                        
                        # TP Multiplier Ratio: 1.0x to 3.0x of the chosen SL (Matches env logic)
                        tp_mult_ratio = 1.0 + ((tp_val + 1.0) * (3.0 - 1.0)) / 2.0
                        
                        # Apply the ratio to the Stop Loss to get final TP multiplier
